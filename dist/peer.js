 ...
 */
import { isRequest, isResponse, isNotification, createRequest, createNotification, createResponse, createErrorResponse, WMPError, } from "./jsonrpc.js";
import { Registry, } from "./profile.js";
import { ErrorCode, Method, VERSION, } from "./types.js";
// ---------------------------------------------------------------------------
// Peer
// ---------------------------------------------------------------------------
export class Peer {
    ...
    constructor(transport, opts) {
        ...
        this.transport.on("error", (err) => this.onTransportError(err));
    ...
    async createSession(opts) {
        const wmp = { version: VERSION, sender: opts.sender };
        const params = {
            wmp,
        ...
        const result = await this.call(Method.SessionCreate, params);
     ...
     */
    bindSessionToTransport(sessionId) {
    ...
    async closeSession(reason = "complete") {
        if (!this.sessionId)
            return;
    ...
    // -------------------------------------------------------------------------
    /** Start a flow. */
    async startFlow(flowType, flowId, params, timeout) {
        return this.call(Method.FlowStart, {
            wmp: this.wmpMeta(),
            flow_type: flowType,
            flow_id: flowId,
            params,
    ...
    async flowProgress(flowId, step, payload) {
    ...
    async flowAction(flowId, action, params) {
        return this.call(Method.FlowAction, {
            wmp: this.wmpMeta(),
            flow_id: flowId,
            action,
            params,
    ...
    async flowComplete(flowId, result) {
    ...
    /** Notify flow error. */
    async flowError(flowId, code, message, data) {
    ...
    /** Cancel a flow. */
    async flowCancel(flowId, reason) {
        return this.call(Method.FlowCancel, {
            wmp: this.wmpMeta(),
            flow_id: flowId,
            reason,
        });
    ...
    // -------------------------------------------------------------------------
    async resolve(type, uri, options) {
        return this.call(Method.Resolve, {
            wmp: this.wmpMeta(),
            type,
            uri,
            options,
    ...
    handleResponse(resp) {
        ...
        clearTimeout(pending.timer);
        if (resp.error) {
            pending.reject(new WMPError(resp.error.code, resp.error.message, resp.error.data));
        }
        else {
    ...
    async dispatchMethod(method, params) {
        ...
        switch (method) {
            ...
            case Method.FlowStart: {
                ...
                if (fh)
                    return fh.startFlow(p);
                if (this.handler.onFlowStart)
                    return this.handler.onFlowStart(p);
                break;
            }
            ...
            case Method.FlowAction: {
                const fh = this.flowHandlerForId(p.flow_id);
                if (fh)
                    return fh.handleAction(p);
                if (this.handler.onFlowAction)
                    return this.handler.onFlowAction(p);
                break;
            }
            ...
            case Method.Resolve: {
                ...
                if (rh)
                    return rh.handleResolve(p);
                if (this.handler.onResolve)
                    return this.handler.onResolve(p);
                break;
            }
            ...
            default: {
                ...
                if (mh)
                    return mh.handleMethod(method, params);
                break;
            }
        }
